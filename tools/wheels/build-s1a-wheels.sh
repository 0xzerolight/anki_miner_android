...
stage="$("$SCRIPT_DIR/s1a_wheels.py" stage \
...
manifest="$("$builder_env/bin/python" "$SCRIPT_DIR/s1a_wheels.py" publish \
    --dist "$chaquopy/server/pypi/dist" --output-root "$output_root")"

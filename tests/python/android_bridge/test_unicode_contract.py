...
def _category_c_truth() -> bytearray:
    ...
    for line in (UNICODE_ROOT / "UnicodeData.txt").read_text(
        encoding="ascii"
    ).splitlines():
        fields = line.split(";")
        ...
        if name.endswith(", First>"):
            ...
            truth[start : code_point + 1] = bytes([is_category_c]) * (
                code_point - start + 1
            )
            pending = None
        else:
...
def _whitespace_truth() -> bytearray:
    document = json.loads(
        (UNICODE_ROOT / "python-3.13-isspace.json").read_text(encoding="utf-8")
    )
...
def _normalization_rows() -> list[tuple[str, str, str, str, str]]:
    ...
    for raw_line in (UNICODE_ROOT / "NormalizationTest.txt").read_text(
        encoding="utf-8"
    ).splitlines():
        ...
        values = tuple(
            "".join(chr(int(token, 16)) for token in field.split())
...
def test_scalar_category_c_and_python_whitespace_match_pinned_truth() -> None:
    ...
    for code_point in range(MAX_CODE_POINT + 1):
        ...
        if is_scalar and unicode_contract.is_category_c(code_point) != bool(
            category_c[code_point]
        ):
            mismatches.append((code_point, "category C"))
        if is_scalar and unicode_contract.is_python_whitespace(code_point) != bool(

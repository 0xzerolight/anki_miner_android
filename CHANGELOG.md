...
- **Undo a run (result card).** Deletes the notes the run created in AnkiDroid and forgets the words it marked known, from the result card on all three mining tabs — desktop has offered this from its results dialog since its own undo shipped, and Android had no delete path at all: the provider gateway was create-only, with a contract test pinning `resolver.delete` absent. The button confirms first, covers cancelled and failed runs that created notes (the engine already preserves partial note IDs for exactly this), and reports the count the provider acknowledged. Undo is host-side only, mirroring desktop's layering — the engine supplies `card_ids` and the `mined_forms` receipt it has carried since the pinned revision, and a process-owned manager deletes per note ID through a new sealed `DeleteNote` mutation, then reverts only the known-words rows that run inserted (`source='mined'`; a lemma the user added themselves is never touched, since mined inserts never downgrade a user row). The delete phase holds the Anki-setup lease so a new run's duplicate scan cannot race a half-deleted collection, and releases it before the known-words phase, which takes the resource lease on its own. Two behaviours were pinned on a real AnkiDroid 2.24.0 rather than assumed: the provider does implement note deletion although `FlashCardsContract` documents only `query()` for notes, and it reports request-count rather than affected-rows — deleting an already-gone note still returns 1 — so the receipt's deleted count carries the same "requested, not verified" semantics desktop ships. An instrumented probe keeps both observations from silently rotting under a future AnkiDroid pin bump.
- **Per-run audio track picker (Video and Audio tabs, below Test timing).** A dual-audio file — a video with a Japanese and a dub track, or a container carrying commentary — mined on whichever stream ffmpeg picked as the container default, with no way to choose otherwise. The new Tracks button probes the file (`media.audiotracks`, a bridge op backed by the vendored ffprobe enumerator already shipped for extraction) and opens a dialog listing every audio stream by language, codec and channel layout; Auto-detect preselects the first Japanese-tagged track (falling back to the first track when none is tagged) and shows which one it currently resolves to. A chosen track rides `audioTrackOverride` through the run to the engine and resets to Auto whenever the source file is replaced.
- **Curation preview plays the picked track.** The preview player maps the chosen stream's ffprobe audio index to Media3's own track-group ordinal rather than assuming group 0, verified against a real dual-audio container fixture; Auto still lets Media3 pick.
- **Curation can widen a sentence with its neighbouring subtitle lines (Video and Audio tabs).** The expanded word row grows three controls — "+ Previous line", "+ Next line", "Reset lines" — that merge adjacent cues into the mined sentence and its audio clip window, preview the merged text and timing from the run's own cue list, and snap the preview player to the widened window's start. The intent rides the curation selection as `linesBefore`/`linesAfter` (1..100, omitted when zero); the bridge stamps it onto a `dataclasses.replace` copy of the live engine word, and the engine's `_materialize_line_expansions` — cherry-picked with `expand_word_lines` from desktop issue #120 onto the pin as tag `android-pin-v2.11.0-line-expansion` (23b361ea + d43a5daf + 19f2a4c1; golden drift provenance-only) — rebuilds sentence text, surface offsets, furigana and timing between curation and phase 3, so extraction and card creation never know expansion existed. Gating is advisory and computed locally, exactly as the desktop dialog does: buttons disable at file edges and at the desktop 30 s clip ceiling (merged window + 2× audio padding), while the engine materializes whatever counts were confirmed. Picking a different example sentence resets the expansion, re-picking the same one keeps it, and counts survive process death inside the curation session state. The reading lane never shows the controls, and the bridge rejects expansion counts on a reading run (`line_expansion_unsupported`) instead of minting silently unexpanded cards.
- **The curation list pins the sentence that will be mined and folds large alternative sets (Video and Reading tabs).** An expanded word rendered every candidate sentence as its own radio row, so a high-occurrence word buried the next candidate under a screen of rows — the vendored engine caps the picker at 12 alternatives today, but the cap is already gone on desktop (`e77d4e3b` offers every compatible line), so the next engine re-pin makes the lists unbounded. Above three alternatives the list now renders the chosen sentence first — the draft's selection, falling back to the engine default — then a "Show N alternatives" disclosure; opened, the alternatives keep subtitle order as separately keyed lazy items so the long-list virtualization contract holds, and tapping one promotes it into the pinned slot while the previous choice re-enters the list at its source position. Disclosure state is scoped to the request and the focused candidate, so it resets per word and per run; at three alternatives or fewer nothing changes. The toggle's count is the pickable-sentence count, never the metadata line's occurrence count, which can run to hundreds while the picker offers at most the engine's cap. Implements IMP-008 from the 2026-08-10 audit (rank 4); the desktop curator has no counterpart — its geometry (a separate scrolling picker pane beside the table) does not transfer to a phone.
- **Manga page context in curation (Reading tab).** Curating a word mined from a `.mokuro` + `.cbz`/`.zip` pair showed only the isolated sentence text, with no way to confirm speaker, panel or visual context before creating a card from an ambiguous occurrence — the vendored engine already carried the source page's image reference and OCR block box, but the wire payload dropped both. Curation sentences from a manga run now carry `imageEntry`/`blockBox`/`locationLabel`, and a pinned page pane renders the source page with the block highlighted and a `p.N` caption, retargeting as the candidate or sentence alternative changes; the pane's collapse state persists across that retargeting. Text and EPUB runs, which have no page image, show no pane. Closes IMP-032.

### Changed

- **The vendored Japanese mining engine is re-pinned to current desktop (`2710117d` -> `ec5e1006`).** The old pin was a cherry-pick branch tip carrying three commits — the line-expansion work and the Yomitan bank-progress callback — that have since landed on desktop `main`, so the new pin is desktop HEAD and loses nothing; 116 mainline commits come with it. Mining output is unchanged where the contract covers it: all three golden fixtures (`engine-v2`, `reading-v1`, `s4-engine-smoke-v1`) re-derive with every semantic section byte-identical and only provenance moved, and `run_head_goldens_v2.py` reports `desktop HEAD semantic drift: none`. What does change for a user: the curation sentence picker is no longer capped at 12 alternatives (the "Show N alternatives" disclosure shipped for this and now sees the unbounded list a word usable on 34 lines actually has); masu-stem nominalizations and honorific-prefix compounds are mined as nouns rather than dropped; a word no source ranks leaves the frequency sort field empty instead of stamping a 9999999 placeholder; kana-front definitions are scoped to the token's lemma (Rule A′); and Yomitan dictionary import reports determinate, monotonic insert progress with a true entry count at the end. Five overlay shadows were rebased over upstream movement and re-reviewed — `models/reading.py`, `services/__init__.py`, `services/media_extractor.py`, `services/tagger.py`, `utils/audio_track_detector.py`; the two upstream additions in that set (the ffmpeg 9 filter-file probe, the container-duration probe) belong to the condenser and the retimer, neither of which is in the Android closure, and the desktop `stdin=DEVNULL` hardening the same commits added was already in both overlays. `anki_miner.services` no longer imports `requests` at package scope, which upstream made lazy.
- **Settings -> Diagnostics bundle replaces Tester diagnostics.** The section carried the only pre-1.0 word left in the product, and it named its audience rather than its contents — the card holds the version, the source commit, a privacy line and the share button, and its sibling keys were already `settings_diagnostics_bundle_privacy` and `settings_share_diagnostics_bundle`, with the bundle's own README calling itself a diagnostics bundle. Renamed in all twelve catalogues; the shared report header drops the same word. Settings search titled this card "Share", which told the user nothing, and now uses the section title. The internal card key stays `tester-diagnostics` — it is not user-visible and renaming it would touch three files to no effect.
- **An untranslated key fails the localization gate.** A key missing from a locale catalogue was an advisory on the reasoning that Android resolves it from `values/`, so the app still renders — which is exactly why it went unnoticed until eleven keys covering a whole onboarding card had shipped as silent English in all eleven locales. A shipped locale that quietly falls back is a defect, so `audit_android_localizations.py` now files it under failures and the advisory machinery goes with it. `app/lint.xml` keeps `MissingTranslation` informational on purpose: Android lint reasons about language rather than region, and this repo ships region-only variants (`values-pt-rBR` with no `values-pt`, `values-zh-rCN`/`values-zh-rTW` with no `values-zh`), so a second differently-modelled gate would add a failure surface without adding coverage.
- **Audio packs join the schema-staleness gate.** A pack left unusable by an index schema bump made cards fall through to the online sources, or carry no audio at all, with nothing said; the re-pinned engine gates on it like dictionaries, frequency and pitch, but only when the processor is handed the scanned registry. The bridge now carries the handle the expression-audio chain already scanned through to the processor — the built chain has dropped every stale pack by then, so a chain-only handoff would have left the new family permanently ungated. `None` when the expression_audio field is unmapped or no pack is enabled, which is exactly when nothing should gate.

### Removed

- **The Anki recovery review list (Settings -> Anki).** A run interrupted mid-mining — process death, mostly — left journaled Anki write operations that startup recovery classified into per-operation review items: one card per stored media file, engineer-language summaries ("exact postchecks failed", "Review-card deck placement needs review"), a manual acknowledgement button on each, and mining disabled until every one was clicked through. A tester hit eleven at once from a single killed run. The items asked the user to attest to journal states the app could neither verify nor repair — the acknowledgement buttons flipped a journal flag and touched nothing in AnkiDroid — so the journal now resolves its own rows at the same finalization boundaries that used to raise them: media claims take the identical typed acknowledgement transition with a system actor recorded in the evidence, uncertain note/deck commits and failed postchecks resolve directly (the engine's first-field duplicate scan makes both commit outcomes safe on the next run), and a claim still referenced by a live note materialization stays open so a resumed note can attach it. Acknowledged claims also release their media namespace reservations, fixing a second fault: an unresolved orphan claim held its namespace across restarts, so re-mining a word whose audio had been stored by an interrupted run died with a journal invariant violation. Mining no longer gates on open review items — only genuine recovery-gate blockage (a persistent staging quarantine) still blocks, with its existing admission message. The remediation rows, their evidence and the terminal audit tables are unchanged and still reach tester diagnostics; existing open items resolve on first launch after the update.
- **The three S1a spike `BuildConfig` flags, and the ARM64 acceptance line they fed.** `S1A_SPIKE_ENABLED` and `S1A_PUBLICATION_VERIFIED` were compile-time constant `true`, so the assertions and the Fake-repository branches they guarded were unreachable in every build; `S1A_ARM64_ACCEPTED` was committed `false` and read only by tester diagnostics, which meant `runtime.device_accepted=false` in every bundle and manifest, real arm64-v8a hardware included, while nothing enforced it — the release-gate task that once did was removed long ago. `FakeReadingMiningRepository` had no other caller and goes with the branch; `FakeMiningRepository` and `FakeMiningRunUndoManager` stay, both constructed directly by tests. With the flag gone the three debug factories became byte-identical to their `app/src/release/` twins and `MiningRepositoryFactoryParityTest` asserted that two identical files were identical, so both source sets fold into `main` — which also removes a dead `BuildConfig` import the release copy of `ReadingRepositoryFactory` had carried unseen, lint having only ever run on the debug variant. Losing its sole `assumeTrue` drops the S1a fixture test out of the instrumentation assumption-gated tuple, re-pinned here; the pinned executed count is unchanged at 335, and the test stays on the UNEXECUTED allowlist because a hosted runner has no `/data/local/tmp` UniDic archive.
- **The AnkiconnectAndroid capability probe (`tools/ankiconnect-fallback/`, `third_party/ankiconnect-fallback/`).** The M0 S2 spike pinned release 1.15 as evidence that an HTTP exporter was reachable, but the port shipped the AnkiDroid ContentProvider seam and the separate loopback word-audio source went in 0.9.0, leaving a verifier script with no caller and a test whose only assertion was that `scripts/health.sh` and the CI workflow still ran it. Both gate steps and the third-party notice paragraph that disclosed the probe go with it; the APK was never downloaded, embedded or distributed, so no attribution obligation is dropped.

### Fixed

- **The curation definition pane and the card it sits beside agree on which lexeme a kana front names.** The re-pinned engine scopes definitions by the token's UniDic lemma (Rule A′) on the card path, but the pane's own lookup went unscoped, so a kana front like ゆう could show one homograph in the pane and write another to the card. The bridge passes the lemma the picker already sends; a blank lemma, or one equal to the mined form, still skips the probe.
- **The Recommended resources block is translated in all eleven languages.** Eleven keys — the card title, description and both button labels, the batch and per-item operation names, the incomplete-install summary, the JPDB and Kanjium resource names, the Add-menu row and the attribution heading — existed only in `values/`, so the block a new user meets in the onboarding wizard's Dictionaries step rendered in English in de, es, fr, in, it, ja, pt-BR, ru, vi, zh-CN and zh-TW, against the README's claim of eleven interface languages. Terminology is taken from each catalogue's own dictionary, pitch and frequency terms, and the established `X of Y` progress pattern is preserved per locale, including the two that drop or widen the separator. The same pass corrects `jitendex_resource_title`, which kept a "recommended" suffix in every locale after the base string dropped it: Jitendex is not in the recommended set, so eleven languages had been labelling it the opposite of the truth.

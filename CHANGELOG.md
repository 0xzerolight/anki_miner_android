...
- Every blank numeric and text field in Settings shows the value it inherits, in grey, as the placeholder. Until now a blank field looked like nothing at all and the only way to learn what the engine would use was to type something and compare the results. The mining tabs already worked this way; Settings now matches them. A test fails the build if the values drift from the engine's own, so a stale placeholder cannot quietly show a wrong number.

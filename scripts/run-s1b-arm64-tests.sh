...
PROVISIONER="${ANKI_MINER_S1B_PROVISIONER:-$SCRIPT_DIR/provision-s1b-test-unidic.sh}"
APP_APK="${ANKI_MINER_S1B_APP_APK:-$REPO_ROOT/app/build/outputs/apk/device/debug/app-device-debug.apk}"
TEST_APK="${ANKI_MINER_S1B_TEST_APK:-$REPO_ROOT/app/build/outputs/apk/androidTest/device/debug/\
app-device-debug-androidTest.apk}"
TEST_CLASS="com.ankiminer.android.tokenizer.MecabNativeTokenizerInstrumentedTest"
TEST_RUNNER="com.ankiminer.android.test/androidx.test.runner.AndroidJUnitRunner"

usage() {
    cat <<'EOF'
Usage: scripts/run-s1b-arm64-tests.sh \
    --serial SERIAL \
    --unidic-dir DIR \
    --page-size 4k|16k \
    --image-fingerprint FINGERPRINT

Runs the S1b production-JNI parity class on one explicitly named arm64 target.
The runner never starts, stops, selects, or accepts an arbitrary device.
EOF
}

fail() {
    echo "S1b arm64 test: $*" >&2
    exit 1
}
...
[[ "$seen_serial" == true && "$seen_dicdir" == true && "$seen_page" == true && "$seen_fingerprint" == true ]] || {
    usage >&2
    exit 2
}
...
PROVISIONER="$(resolve_command "$PROVISIONER" 'S1b UniDic provisioner')"

device_state="$("$ADB_COMMAND" -s "$serial" get-state 2>/dev/null)" || fail "cannot query target $serial"
...
ANKI_MINER_ADB_COMMAND="$ADB_COMMAND" ANDROID_SERIAL="$serial" \
